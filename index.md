## Projetos selecionados em Ciência de Dados e Bioinformática

---

### neuriteScan: Pacote R para análise de imagens de neuritos 

   Pacote R que tem como entrada imagens de fluorescência de células neuronais e retorna ao usuário a densidade de neuritos contidos na imagem. O desenvolvimento desse pacote fez parte da minha dissertação de mestrado e possui como um dos diferenciais a entrada de dados high throughput possibilitando a otimização da análise. Além disso, o neuriteScan faz integração das linguagens R e Python através do pacote R reticulate, a fim de agregar diferentes ferramentas de processamento de imagem.

   <img src="images/neuritescan.png?raw=true" />

[![](https://img.shields.io/badge/R-white?logo=R)](#) [![](https://img.shields.io/badge/Python-white?logo=Python)](#) [![](https://img.shields.io/badge/NumPy-white?logo=data:image/png;base64,)](#) 

[Documentação do pacote](https://github.com/sysbiolab/neuriteScan)

[Dissertação de mestrado no acervo UFPR](https://acervodigital.ufpr.br/handle/1884/87277)
   
---

### Análise de dados lncRNAs de câncer de mama (TCGA-BRCA)

   Análise bioinformática de dados long non-coding RNAs (lncRNAs) de câncer de mama. Fui a responsável pela análise completa dos dados da coorte de câncer de mama (BRCA) provenientes do The Cancer Genome Atlas (TCGA), desde a obtenção dos dados, manipulação e pré-processamento, análise de expressão diferencial e confecção do heatmap (Figura 3 do artigo). Essa publicação foi parte de uma colaboração que fiz durante o doutorado, onde estive em contato direto com pesquisadores, geneticistas e professores. Além de fazer a análise bioinformática completa, contribui para a escrita da seção de metodologia e resultados do artigo, o qual foi publicado em 2023. 

<img src="images/lncrna2.png?raw=true" />

[![](https://img.shields.io/badge/R-white?logo=R)](#)

[Artigo publicado na Cancers (2023)](https://doi.org/10.3390/cancers15194682)

---

Página em construção. Em breve, outros projetos serão adicionados.




---
<!-- <p style="font-size:11px">Page template forked from <a href="https://github.com/evanca/quick-portfolio">evanca</a></p>
<!-- Remove above link if you don't want to attibute -->
